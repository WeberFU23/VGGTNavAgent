...
# semantic_memory 后端还需 vLLM 服务与三个模型权重（见
...
for backend in clip_sam semantic_memory; do
  # 混合模式：聚合输出里看 sr_mos / sr_moc / f1 / spl_multi
  NAV_SEMANTIC_BACKEND=$backend \
    run_eval "${backend}_mixed" --limit 1 --episode-limit "$EPISODE_LIMIT"
  # 红线 episode：any basket，F1=1.0 不许回退
  NAV_SEMANTIC_BACKEND=$backend \
    run_eval "${backend}_redline" --limit 1 --episode-id "$EPISODE_REDLINE"
done

echo
echo "smoke 矩阵完成，日志在 $OUT_ROOT/。"
echo "对比两后端的 sr/f1/spl_multi（many/all 主榜单）；红线：${EPISODE_REDLINE} F1=1.0。"
