...
状态：待审核。本文档描述目标设计，**不代表当前代码状态**（当前架构见
`AGENT_ARCHITECTURE.md`）。已提前落地的基础改动：world-state 精简
...
2. **VLM 永不输出坐标**：所有坐标、深度、路径代价由工具返回并来自
   VGGT/几何模块。VLM 只引用 ID（frame_id、instance_id、frontier_id）。
...
4. **覆盖可兜底**：frontier 机制保留，几何/语义覆盖层继续由确定性模块
   维护并呈现在鸟瞰图上；覆盖统计经 `get_map_status` 按需查询；VLM 不
   探索时系统能检测停滞。
5. **记忆一致性由 harness 保证**：实例合并/撤销、reported 状态、
   candidate 重投影仍走现有工具约束，VLM 不能直接改 3D 坐标。
...
| 工具 | 说明 |
|---|---|
| `search_captions(text)` | BGE 检索关键帧 caption |
| `search_instances(keywords, reported, top_k)` | 实例文本关键词检索 |
| `inspect_instance(instance_id)` | 实例完整记录 |
| `update_instance / merge_instances / undo_merge` | 记忆维护 |

### 3.2 改造的工具

| 工具 | 现状 | 改造 |
|---|---|---|
| `look_instance(instance_id)` | 返回 pointing evidence crop | 保留，证据图照旧 |
| `SCAN` | 固定 12 步环视动作 | 保留为**动作**（改变朝向，不是纯信息工具），但不再有任何路径强制触发，VLM 自主决定何时环视 |
| `START_ADJUST` | 事件内进入微调 | 保留语义，更名 `takeover()`：VLM 显式接管，进入有界离散动作模式（白名单 `MOVE_FORWARD/TURN_LEFT/TURN_RIGHT`，`NAV_ADJUST_MAX_STEPS` 上限不变），`end_takeover()` 退出。**takeover 期间禁止工具调用**：每轮只出动作+收新 RGB，保持反应式控制节奏 |
...
| `instantiate(frame_id, description)` | 帧 ID + 目标描述 | `[{instance_id, point, confidence, bbox, depth_std}]` | 把 `ground_object` 拆开：跳过 BGE 检索，直接对指定帧 pointing + bbox 约束采样 + 注册 candidate + 写入 InstanceMemory。server 新增 `point_frame` RPC |
| `ground_text(description)` | 自由文本 | 同现有 `ground_object` 全流程（BGE→pointing→3D） | 现有能力包成工具，供 VLM 在不知道 frame 时一把梭 |
| `get_map_status()` | 无 | 覆盖统计：已探索面积、frontier 数、caption pending、剩余步数预算、最新关键帧编号 | 聚合现有 `get_state` + world-state 字段；帧编号让 VLM 不等通知也能发现可查的帧 |
| `update_notes(text)` | 自由文本（上限约 500 字） | 更新后的 notes | VLM 自己维护的跨决策工作记忆：当前计划、已知结论、待办。harness 只存一个字符串，随每次决策注入。类比 coding agent 的 TODO/scratchpad，但内容是 VLM 提炼的结论，不是原始对话 |
| `get_action_history(before_step, limit)` | 分页参数 | `[{step, action, target_id, outcome}]` | 全量动作流水（harness 确定性记录并落盘），不主动下发，VLM 需要回顾早期经历时按需查询 |

工具分层意图：`ground_text` 是高层便捷工具（一步完成检索+定位），
`search_captions → view_frame → instantiate` 是分解后的精细工具链。VLM
按情况选择：目标明确且着急用前者；需要核实、多目标、或检索结果可疑时
用后者逐步确认。**这正是"很多步骤不一定必要"的落地方式。**

重复实例是预期行为：同一区域多次 `instantiate`/`ground_text` 可能为同一
物体生成多个实例，由 VLM 用 `merge_instances` 合并（与现状一致），
harness 不做基于距离/类别的自动合并。
...
- **VLM 的工作记忆**：`notes` 字段随 world-state 下发，VLM 用
  `update_notes` 自行改写，维护"我在做什么、已排除什么、下一步打算"
  这类提炼后的结论；
...
- **例外二（报告亲见约束）**：prompt 契约要求 `REPORT_FOUND` 前必须
  ...
  是否硬校验待评测后定；
- **caption 同步返回**：`search_captions` / `ground_text` / 移动完成的
...
- 工具调用轮次保留 `NAV_DECIDER_MAX_TOOL_ROUNDS` 上限；超过上限强制
  要求输出动作；
- world-state 增加预算与记忆字段：`steps_remaining`（相对
  ...
  world-state，经 `get_map_status` 按需查询；
...
| 现状行为 | 改造后 |
|---|---|
| 探索中按 `NAV_QUERY_INTERVAL` 定时 ground 目标 | 删除；VLM 自主决定何时 `ground_text`/`instantiate` |
| SCAN 后强制刷新 caption/pointing/实例 | caption 本就自动；实例刷新改为 VLM 调用工具触发 |
| 到达后固定 evidence 展示 + 动作白名单 | 到达事件只带 candidate 编号与摘要，图像由 VLM 用 `view_frame`/`look_instance` 按需取（报告亲见约束见 §3.6）；动作不限死 |
...
1. **server 加 `point_frame` RPC**（对指定帧 pointing + 实例化，复用
   ...
   client 加对应方法；单测覆盖。
2. **决策层加 `view_frame` / `instantiate` / `ground_text` /
   `get_map_status` 四个工具**，接入 `agent_loop` 工具循环；单测覆盖。
3. **观察编号制落地**：移动完成后向决策事件附新关键帧编号摘要；
   `search_captions`/`ground_text`/编号通知改为返回前等待 caption 队列
   清空（有界）；world-state 加预算与覆盖字段；prompt 重写为 harness
   风格（先写 prompt 草案给用户审）。
4. **删除定时 ground 管线**（`NAV_QUERY_INTERVAL` 路径），放宽事件
   动作白名单；保留 `ground_top_k` 等配置给 `ground_text` 用。
