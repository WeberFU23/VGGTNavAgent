#!/usr/bin/env bash
# 5-episode validation run (fresh episodes, no overlap with previous runs):
# 语义校验修正闭环 + adjust 前进近距离点击 + current_frame_id 曝光。
# any:  6s7QHgap2fW_ep0000001 (bath) / wcojb4TFT35_ep0000002 (bicycle)
# many: k1cupFYWXJ6_ep0000004 (4x bench, FP 历史类别) /
#       TEEsavR23oF_ep0000013 (4x wardrobe, incomplete_finish 历史类别) /
#       5cdEh9F2hJL_ep0000001 (3x couch, 新场景)
set -u

PROJECT=/root/autodl-tmp/vggt_nav_agent
BENCH=/root/autodl-tmp/habitat_benchmark_eval_20260827
RUN_ROOT=/root/autodl-tmp/runs_5ep_20260830
...
EPS="
k1cupFYWXJ6_ep0000004
TEEsavR23oF_ep0000013
wcojb4TFT35_ep0000002
6s7QHgap2fW_ep0000001
5cdEh9F2hJL_ep0000001
...
for EP in $EPS; do
  ...
  screen -dmS nav-mapping-5ep bash -lc \
