#!/usr/bin/env bash
# smoke10_v7 冒烟测试：v7 benchmark + 最新 agent（HEAD ec08626 同步后），
# 10 个分层随机 ep（category 4 / attribute 4 / image 2，many/all 混合，10 个不同场景）。
# many/all 步数预算由 benchmark 常量 MANY_ALL_STEP_BUDGET=800 决定（CLI --max-steps 无效于 many/all）。
set -u
PROJECT=/root/autodl-tmp/vggt_nav_agent
BENCH=/root/autodl-tmp/habitat_benchmark_eval_20260827
RUN_ROOT=/root/autodl-tmp/runs_smoke10_v7_20260908
DATASET="$BENCH/benchmark_v7/semantic"
SCENE_ROOT=/root/autodl-tmp/datasets/hm3d/val
PORT=5555
# scene 不重复；image 两个用 --goal-type image
declare -A GOAL_TYPE=(
  [a8BtkwhxdRV_ep0000009]=description
  [bCPU9suPUw9_ep0000001]=description
  [cvZr5TUy5C5_ep0000018]=description
  [q3zU7Yy5E5s_ep0000012]=description
  [q5QZSEeHe5g_ep0000013]=description
  [svBbv1Pavdk_ep0000004]=description
  [yr17PDCnDDW_ep0000017]=description
  [zt1RVoi7PcG_ep0000004]=description
  [DYehNKdT76V_ep0000020]=image
  [mv2HUxq3B53_ep0000020]=image
)
EPS="a8BtkwhxdRV_ep0000009 bCPU9suPUw9_ep0000001 cvZr5TUy5C5_ep0000018 q3zU7Yy5E5s_ep0000012 q5QZSEeHe5g_ep0000013 svBbv1Pavdk_ep0000004 yr17PDCnDDW_ep0000017 zt1RVoi7PcG_ep0000004 DYehNKdT76V_ep0000020 mv2HUxq3B53_ep0000020"
...
for EP in $EPS; do
  ER="$RUN_ROOT/$EP"
  mkdir -p "$ER"
  echo "[run] $(date) === $EP (${GOAL_TYPE[$EP]}): start mapping server ===" | tee -a "$RUN_ROOT/run.log"
  bash -c "
    source /root/miniconda3/etc/profile.d/conda.sh
  ...
  python -u evaluation/main/run_eval.py \
    ...
    --episode-id "$EP" --goal-type "${GOAL_TYPE[$EP]}" \
